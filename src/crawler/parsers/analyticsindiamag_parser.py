"""Analytics India Magazine Parser - Extracts article content from Analytics India Magazine"""
...
class AnalyticsIndiaMagParser(BaseParser):
    """Parser for Analytics India Magazine articles"""
    ...
    def __init__(
        ...
        self.logger.info(f"AnalyticsIndiaMagParser initialized")

    def _get_logger_name(self) -> str:
        return "analyticsindiamag_parser"

    def parse_single(self, url: str) -> Dict[str, Any]:
        """Parse a single Analytics India Magazine article"""
        self.logger.info(f"Parsing Analytics India Magazine article: {url}")
    ...
    def _extract_content_from_html(self, soup: BeautifulSoup) -> str:
        # Try entry-content div first (common in WordPress/magazine sites)
        article = soup.find('div', class_='entry-content')
        if not article:
            article = soup.find('div', class_=lambda x: x and ('post-content' in str(x).lower() or 'article-content' in str(x).lower()))
        if not article:
    ...
    def parse_multiple(self, urls, output_dir: str = None):
        """Not implemented for Analytics India Magazine parser"""
        raise NotImplementedError("Analytics India Magazine parser does not support batch processing")

    def save_results(self, results, output_file: str):
        """Not implemented for Analytics India Magazine parser"""
        raise NotImplementedError("Analytics India Magazine parser does not support saving results")
