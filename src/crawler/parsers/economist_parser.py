"""The Economist Parser - Extracts article content from The Economist
...
class EconomistParser(BaseParser):
    """Parser for The Economist articles"""
    ...
    def __init__(
        ...
        self.logger.info(f"EconomistParser initialized with cloudscraper")

    def _get_logger_name(self) -> str:
        return "economist_parser"

    def parse_single(self, url: str) -> Dict[str, Any]:
        """Parse a single The Economist article"""
        self.logger.info(f"Parsing The Economist article: {url}")
    ...
    def _extract_author(self, data: dict) -> str:
        if 'author' in data:
            ...
            if isinstance(author, dict):
                return author.get('name', 'The Economist')
            elif isinstance(author, list) and len(author) > 0:
                if isinstance(author[0], dict):
                    return author[0].get('name', 'The Economist')
                return str(author[0])
            return str(author)
    ...
    def parse_multiple(self, urls, output_dir: str = None):
        """Not implemented for The Economist parser"""
        raise NotImplementedError("The Economist parser does not support batch processing")

    def save_results(self, results, output_file: str):
        """Not implemented for The Economist parser"""
        raise NotImplementedError("The Economist parser does not support saving results")
